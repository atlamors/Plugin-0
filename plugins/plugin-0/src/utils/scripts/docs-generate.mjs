...
const root = path.resolve(process.cwd(), "plugins/plugin-0")
const readmePath = path.join(root, "README.mdx")
const configPath = path.join(root, "src/config.ts")

function extractRoutes() {
  const routes = []
  const base = fs.existsSync(path.join(root, "src/api")) ? path.join(root, "src/api") : path.resolve("plugins/plugin-0/src/api")
  const walk = (dir) => {
...
function extractOptions() {
  try {
    ...
    const match = src.match(/Plugin0OptionsSchema\s*=\s*z\.object\(\{([\s\S]*?)\}\)/)
    if (!match) return []
    const body = match[1]
    const lines = body
      .split(/\n/)
